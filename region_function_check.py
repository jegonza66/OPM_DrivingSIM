...
standarize_normalize_con = 'std'  # std / norm Standarize connectivity matrices within subjects
...
for epoch_id in epoch_ids:
    ...
    run_path_plot = run_path.replace('Band_None', f"Band_{meg_params['band_id']}")
    ...
    label_ts_save_path = paths.save_path + f"Source_labels_{meg_params['data_type']}/{run_path}/" + labels_model_path

    # Connectivity matrices plots and save paths
    fig_path = paths.plots_path + f"{main_path}_{meg_params['data_type']}/" + run_path_plot + source_model_path + f"_{parcelation_segmentation}_{final_path}_{standarize_normalize_con}/"
    ...
    for subj_num, subject_id in enumerate(exp_info.subjects_ids):
        ...
        if os.path.isfile(fname_con):
            ...
            # Load MEG data
            if not envelope_connectivity:
                meg_params['band_id'] = None
            meg_data = load.meg(subject_id=subject_id, task=task, meg_params=meg_params)
            ...
            if os.path.isfile(label_ts_save_path + labels_ts_data_fname):
                ...
                if os.path.isfile(epochs_save_path + epochs_data_fname):
                    ...
                    # Epoch data
                    data_epochs, events, onset_times = functions_analysis.epoch_data(subject=subject, epoch_id=epoch_id,
                ...
                if len(data_epochs) == 1 and 'annot' in meg_params['data_type'] and onset_times != None:
                    # --------- EXTRA ---------#
                    ...
                    for annot in data_epochs.annotations:
                        if annot['description'] == 'BAD':  # Only consider BAD annotations
                            start = max(annot['onset'] - onset_times[0] - meg_data.first_time,  0)  # Adjust to epoch time
                            end = min(annot['onset'] + annot['duration'] - onset_times[0], data_tmax - data_tmin)
                ...
                stc_epochs = beamformer.apply_lcmv_epochs(epochs=data_epochs, filters=filters, return_generator=False)
            ...
            if envelope_connectivity:
                if downsample_ts:
                    for i, ts in enumerate(label_ts):
                        samples_interval = int( meg_data.info['sfreq']/desired_sfreq)
...
if len(epoch_ids) > 1:
    ...
    # for epoch_id in epoch_ids:
        ...
        if t_values.any() > 0:
            ...
            # Plot circle
            plot_general.connectivity_circle(subject='GA', labels=fsaverage_labels, surf_vol=surf_vol, con=t_values, connectivity_method=connectivity_method,
            ...
            # Plot matrix
            sig_diff_sorted_matrix = plot_general.plot_con_matrix(subject='GA', labels=fsaverage_labels, adjacency_matrix=t_values, subject_code='fsaverage',
        ...
        # Plot matrix
        diff_sorted_matrix = plot_general.plot_con_matrix(subject='GA', labels=fsaverage_labels, adjacency_matrix=con_diff_ga, subject_code='fsaverage',
