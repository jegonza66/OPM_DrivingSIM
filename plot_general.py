...
def mri_meg_alignment(subject, subject_code, dig, subjects_dir=os.path.join(paths.mri_path, 'FreeSurfer_out')):
    ...
    import load
    meg_data = load.meg(subject_id=subject.subject_id)

    # Visualize MEG/MRI alignment
    surfaces = dict(brain=0.7, outer_skull=0.5, head=0.4)
    # Try plotting with head skin and brain
    try:
        mne.viz.plot_alignment(meg_data.info, trans=trans_path, subject=subject_code,
...
def plot_trf_features(grand_avg,
    ...
    for i, coeff in enumerate(grand_avg.keys()):
        ...
        ax_frp = fig.add_axes((left, 0.62, width, 0.2))
        ax_tfce = fig.add_axes((left, 0.42, width, 0.2))
        ax_tfce_topo = fig.add_axes((left + (width - width_topo)/2, 0.1, width_topo, 0.2))
        ax_tfce_topo_cb = fig.add_axes((left + width - tfce_topo_cb_width * 2, 0.075, 0.006, 0.145))
